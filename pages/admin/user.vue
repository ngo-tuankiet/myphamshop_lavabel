<template>
    <div>
        <a-card title="Quản lý tài khoản">
                ...
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.key === 'action'">
                        <a-space>
                            ...
                            <a-popconfirm title="Xóa tài khoản này?"
            ...
            <!-- MODAL THÊM / SỬA -->
            <a-modal :title="modalTitle" v-model:visible="visible"
                @ok="handleOk"
                @cancel="handleCancel"
...
// ======================== CONFIG ===========================
const API_BASE = "http://127.0.0.1:8000/api/admin/users";
...
const fetchUsers = async () => {
    try {
        ...
        users.value = json.data.data; // paginate format
    } catch (err) {
...
const handleOk = () => {
    formRef.value.validate().then(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                message.error(json.error || "Lỗi xử lý tài khoản");
